...
intro_plot2 <-
  plot_grid(
    # Left hand side
    plot_grid(region_plots$N, 
            region_plots$SW,
            ncol = 1, nrow = 2),
    # Middle
    plot_grid(aus_map, 
              plot_grid(NULL, region_plots$TAS, NULL,
                        nrow = 1, ncol = 3,
                        rel_widths = c(0.5, 1, 0.5)),
              nrow = 2, ncol = 1,
              rel_heights = c(1, 0.4)),
    # Right hand side
    plot_grid(region_plots$S, 
            region_plots$SE,
            ncol = 1, nrow = 2),
    nrow = 1, ncol = 3,
    rel_widths = c(0.5, 1, 0.5),
...
ggsave(plot = intro_plot2, 
       paste0(dir, "/figures/intro_plot.png"), device = "png", 
       width = 180,  units = "mm", 
       dpi = 300)
```
...
rls_means_region_plot <- 
  ...
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = ifelse(rls_means_region$limit == "northern", -Inf, 0), ymax = ifelse(rls_means_region$limit == "northern", 0, Inf)), fill = "lightgrey", alpha = 0.3, col = NA) +
  geom_point(position = position_dodge(width = 0.3)) +
  geom_errorbar(aes(ymin = dLat_mean - dLat_se,
                    ymax = dLat_mean + dLat_se),
                width = 0.2, position = position_dodge(width = 0.3)) +
  ...
  geom_text(aes(x = "TAS", y = 1),
            label = ifelse(rls_means_region$limit == "northern", "extending", "contracting"), 
            col = "black",
            size = 5) +
  geom_text(aes(x = "TAS", y = -1),
            label = ifelse(rls_means_region$limit == "northern", "contracting", "extending"), 
            col = "black",
            size = 5) +
  facet_grid(limit ~.) +
