...
ggarrange(rls_summary_bp, rls_means_plot_wt, rls_summary_regression,
          nrow = 3, ncol = 1)
```
...
aus_map <- ggplot() +
  ...
  labs(fill = "dSST per decade (°C)", x = NULL, y = NULL) +
  theme_bw() +
  theme(legend.position = "bottom", legend.direction = "horizontal")  

# Regional means
rls_means_region <- rls_summary[region != "TAS", 
                                .(N = NROW(dLat),
                                  dLat_mean = mean(dLat),
                                  dLat_sd = sd(dLat),
                                  dLat_se = sd(dLat) / sqrt(NROW(dLat)),
...
# Test means are different to 0 with ANOVA
fit <- aov(dLat ~ 1 + guild + limit + region, data = rls_summary)

library(multcomp)

summary(glht(fit, linfct = mcp(guild = "Dunnett"))) # Dunnet or Tukey?
...
for (i in 1:length(regions)) {
  ...
  # Create dotplot plot
  p2 <- ggplot(rls_means_region[region == regions[i] & N > 30], 
                       aes(x = guild, y = dLat_mean, col = guild)) +
    geom_point(position = position_dodge(width = 0.3)) +
    geom_errorbar(aes(ymin = dLat_mean - dLat_se,
                      ymax = dLat_mean + dLat_se),
...
# Create NA_regression plot
NA_regression <- ggplot(rls_summary[N > 50 & region == "NA"], 
                        aes(x = dSST, y = dLat, col = guild)) +
  geom_smooth(method = "glm") + 
  geom_hline(yintercept = 0, lty = "dashed") +
  scale_color_manual(values = guild_cols) +
...
# Create NA_dotplot plot
NA_dotplot <- ggplot(rls_means_region[region == "NA" & N > 10], 
                     aes(x = guild, y = dLat_mean, col = guild)) +
  geom_point(position = position_dodge(width = 0.3)) +
  geom_errorbar(aes(ymin = dLat_mean - dLat_se,
                    ymax = dLat_mean + dLat_se),
...
# Set common y-axis limits based on the maximum range of both plots
y_limits <- range(c(layer_scales(NA_regression)$y$range$range,
                    layer_scales(NA_dotplot)$y$range$range))

# Update NA_regression plot with common y-axis limits
NA_regression <- NA_regression +
  ylim(y_limits)

# Update NA_dotplot plot with common y-axis limits
NA_dotplot <- NA_dotplot +
  ylim(y_limits)

# Combine the two plots using cowplot
library(cowplot)

NA_plot <- plot_grid(NA_regression, NA_dotplot,
                     labels = c("A", "B"),
                     ncol = 2,
                     align = "h",
                     axis = "lr",
...
# WA plot
WA_regression <- ggplot(ext_sites[N > 50][region == "WA"], 
                        aes(x = dSST, y = dLat, col = guild)) +
  geom_smooth(method = "glm") + 
  geom_hline(yintercept = 0, lty = "dashed") +
  scale_color_manual(values = guild_cols) +
...
WA_dotplot <- ggplot(rls_means_region[region == "WA" & N > 10], aes(x = class, y = dLat_mean, col = guild)) +
  geom_point(position = position_dodge(width = 0.3)) +
  geom_errorbar(aes(ymin = dLat_mean - dLat_se,
                    ymax = dLat_mean + dLat_se),
                width = 0.2, position = position_dodge(width = 0.3)) +
...
WA_plot <- ggarrange(WA_regression, WA_dotplot,
                     nrow = 1, ncol = 2,
                     widths = c(2, 1)) #THERE IS SOMETHING WEIRD IN THIS PLOT; CHECK WITH ORIGINAL REGRESSION PLOTS
...
# Add movement factor based on 0.5 degrees latitudinal change
fish_traits[, move:= ifelse(abs(dLat) < 0.5, "non-mover", "mover")]
fish_traits$move_num <- as.numeric(as.factor(fish_traits$move))
...
plot(fish_traits[, .(dLat, dSST, range, max_length, max_depth, sgi, therm_max, guild, presence, trophic_group, water_column, move)]) # raw data
...
fish_cor_matrix     <- cor(fish_traits[,.(range, max_length, max_depth, sgi, therm_max, guild_num, presence_num, trophic_group_num, water_column_num, move_num)])
...
traits <- c("guild", "presence", "range_scaled_factor", "max_length_factor", "max_depth_factor", "therm_max_factor", "sgi_scaled_factor", "trophic_group", "water_column", "move")
