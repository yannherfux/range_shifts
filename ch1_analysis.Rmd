...
rls_summary <- rls_summary[, ':=' (guild = factor(guild, levels = c("tropical", "temperate")),
                                   ...
                                   limit = factor(ifelse(limit == "northern", "warm", "cool"), levels = c("warm", "cool")))]
```
...
# Show average temperature as a fractional change of the starting temperature
...
for(i in site_sst_yearly[, unique(region)]){
  ...
  p1 <- ggplot(data_p1, aes(x = year, y = regional_mean)) + 
    geom_line(col = "red") +
    geom_vline(xintercept = ifelse(i == "N", 2016, ifelse(i == "SW", 2011, 2008)), 
               lty = "dashed", alpha = ifelse(i %in% c("N", "SW"), 1, 0)) +
    labs(y = "Mean SST (fold)", x = "") +
...
for(i in rls_summary[, unique(region)]){
  ...
  p1 <- ggplot(data_p1[N > 3],
               aes(y = lat1, fill = dir)) + 
    geom_histogram(binwidth = 2, boundary = 1, position = "dodge") +
    #scale_fill_manual(values = c("gray31", "grey59")) +
...
intro_plot2 <-
  plot_grid(
    # Left hand side
    plot_grid(plot_grid(region_plots$N,
                        site_sst_plots$N,
                        ncol = 1, nrow = 2,
                        rel_heights = c(1, 0.3)),
              plot_grid(region_plots$SW,
                        site_sst_plots$SW,
                        ncol = 1, nrow = 2,
                        rel_heights = c(1, 0.3)),
              ncol = 1, nrow = 2),
    # Middle
    plot_grid(aus_map, 
              plot_grid(NULL, plot_grid(region_plots$TAS,
                                        site_sst_plots$TAS,
                                        ncol = 1, nrow = 2,
                                        rel_heights = c(1, 0.5)), NULL,
                        nrow = 1, ncol = 3,
                        rel_widths = c(0.5, 1, 0.5)),
              nrow = 2, ncol = 1,
              rel_heights = c(1, 0.5)),
    # Right hand side
    plot_grid(plot_grid(region_plots$S,
                        site_sst_plots$S,
                        ncol = 1, nrow = 2,
                        rel_heights = c(1, 0.3)),
              plot_grid(region_plots$SE,
                        site_sst_plots$SE,
                        ncol = 1, nrow = 2,
                        rel_heights = c(1, 0.3)),
              ncol = 1, nrow = 2),
...
rls_summary_regression <- ggplot(rls_summary, aes(x = dSST, y = dLat, col = guild)) +
  ...
  geom_text(data = rls_summary[limit == "warm" & guild == "tropical"],
            aes(x = -2.7, y = -6, label = "p < 0.05"),
            hjust = 0, vjust = -1, color = "black") +
  geom_text(data = rls_summary[limit == "cool" & guild == "tropical"],
            aes(x = -2.7, y = -13, label = "p = 0.05"),
            hjust = 0, vjust = -1, color = "black") +
  geom_text(data = rls_summary[limit == "warm" & guild == "temperate"],
            aes(x = -2.7, y = -6, label = "p = 0.17"),
            hjust = 0, vjust = -1, color = "black") +
  geom_text(data = rls_summary[limit == "cool" & guild == "temperate"],
            aes(x = -2.7, y = -13, label = "p = 0.69"),
            hjust = 0, vjust = -1, color = "black") +
  
  # Add text for slope values
  geom_text(data = rls_summary[limit == "warm" & guild == "tropical"],
            aes(x = -2.7, y = -9, label = "slope = -0.70"),
            hjust = 0, vjust = -1, color = "black") +
  geom_text(data = rls_summary[limit == "cool" & guild == "tropical"],
            aes(x = -2.7, y = -18, label = "slope = -0.20"),
            hjust = 0, vjust = -1, color = "black") +
  geom_text(data = rls_summary[limit == "warm" & guild == "temperate"],
            aes(x = -2.7, y = -9, label = "slope = -0.14"),
            hjust = 0, vjust = -1, color = "black") +
  geom_text(data = rls_summary[limit == "cool" & guild == "temperate"],
            aes(x = -2.7, y = -18, label = "slope = -0.03"),
            hjust = 0, vjust = -1, color = "black") +
...
rls_means_region_plot <- 
  ggplot(rls_means_region[N > 10],
    ...
    geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = ifelse(rls_means_region$limit == "warm", -Inf, 0), ymax = ifelse(rls_means_region$limit == "warm", 0, Inf)), fill = "lightgrey", alpha = 0.3, col = NA) +
    geom_point(position = position_dodge(width = 0.3)) +
    geom_errorbar(aes(ymin = dLat_mean - dLat_se,
                      ymax = dLat_mean + dLat_se),
                  width = 0.2, position = position_dodge(width = 0.3)) +
    ...
    geom_text(aes(x = "TAS", y = 1),
              label = ifelse(rls_means_region$limit == "warm", "extending", "contracting"), 
              col = "black",
              size = 5) +
    geom_text(aes(x = "TAS", y = -1),
              label = ifelse(rls_means_region$limit == "warm", "contracting", "extending"), 
              col = "black",
              size = 5) +
    facet_grid(limit ~.) +
    ...
    scale_y_continuous(breaks = seq(-1, 1, by = 0.5)) +  # Add 0.5 interval marks on the y-axis
    theme_bw() +
    theme(axis.title.x = element_blank(),
          panel.grid = element_blank(),
          panel.spacing = unit(2, "lines")) +  # Adjust the panel spacing
    guides(colour = "none")
...
anom_sites_plot <- 
  ggplot(anom_sites, 
       aes(x = year, y = mean_sst, col = site_code)) + 
  geom_line() + 
