...
for(i in site_sst_yearly[, unique(region)]){
  ...
  p1 <- ggplot(data_p1, aes(x = year, y = regional_mean)) + 
    ...
    labs(y = "Temp anomaly (°C)", x = "", subtitle = ifelse(i == "N", "a",
                                                            ifelse(i == "SW", "b",
                                                                   ifelse(i == "S", "c", 
                                                                          ifelse(i == "SE", "d", "e"))))) +
...
# Horizontal
summary_plot <- 
  ggarrange(ggarrange(rls_summary_vp + theme(strip.text = element_blank()), rls_means_plot_wt + theme(strip.text = element_blank()),
                    nrow = 1),
          rls_summary_regression,
          ncol = 2)
...
# Add restraints cateogry to subset
rls_means_region$restriction <- ifelse(rls_means_region$region == "S" | rls_means_region$region == "TAS", "biogeographical restriction",
                                       ifelse(rls_means_region$region == "N" & rls_means_region$limit == "warm edge", "sampling restriction", "no restriction")) 
rls_means_region$restriction <- factor(rls_means_region$restriction, levels = c("no restriction", "sampling restriction", "biogeographical restriction"))
...
rls_means_region_plot <- 
  ggplot(rls_means_region[N > 10],
       aes(x = region, y = dLat_mean, col = guild, shape = restriction)) +
