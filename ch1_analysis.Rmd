...
rls_sites <- rls[period == "preheatwave",
                 ...
                 by = .(site_code, latitude, longitude, ecoregion)][rls[period == "postheatwave",
...
# Add dSST and ecoregion data
rls_summary <- rls_summary[rls_sites[, .(ecoregion, dSST), by = .(site1 = site_code)], on= .(site1),
                           nomatch = NULL] #nrow = 2,364
...
dLat_plot <- ggplot() +
  ...
  geom_point(data = rls_summary, 
             aes(x = lon, y = lat, shape = guild),
             size = 3) +
  
  # Select shapes to display
  scale_shape_manual(values = c("tropical" = 1, "temperate" = 2)) +
