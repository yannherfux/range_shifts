...
```{r}
path <- "C:/Users/yherrera/OneDrive - University of Tasmania/Desktop/UTas/R/chapter1"

setwd("C:/Users/yherrera/OneDrive - University of Tasmania/Desktop/UTas/R/chapter1")
```
...
rls.atrc <- fread("Raw Data/aus-rls-data.csv") #nrows = 1,515,420

# RLS Method 1 2021 update
rls.2021 <- fread("Raw Data/Aus_RLS_M1_20210903.csv") #nrows = 820,516
...
sst_data <- fread("Raw Data/rls_sites_sst_2yr.csv") #nrows = 26,904
```
...
rls <- rls[, c(1:2, 5:19)][rls_sites, on= .(site_code)] 
```
...
rls <- rls[, .(total = sum(total)), 
           by = .(survey_id, taxon, class, guild, max_length, mobility, site_code, latitude, longitude, survey_date, period, mean_sst, side)] #nrow = 409,791
```
...
# Discard species in Norfolk Island and Christmas Island
...
rls <- rls[, lapply(.SD, mean), # calculate means for the specified data cols
    by = .(taxon, class, guild, max_length, presence, mobility, site_code, latitude, longitude, period, side), # subsets
...
rls_summary <- rls[period == "preheatwave" & 
                   ...
                   .(lat1 = unique(lat), lon1 = unique(lon), site1 = unique(site), 
                     temp1 = unique(temp), 
                     total1 = mean(total)),
                   by = .(taxon, class, guild, max_length, presence, mobility, side, limit, method)][rls[period == "postheatwave" & 
                                                                                                           latitude > (lat - 0.5) & latitude < (lat + 0.5), # approximate latitude to limit data 
                                                                                                         .(lat2 = unique(lat), lon2 = unique(lon), site2 = unique(site), 
                                                                                                           temp2 = unique(temp), 
                                                                                                           total2 = mean(total)),
                                                                                                         by = .(taxon, class, guild, max_length, presence, mobility, side, limit, method)], on = .(taxon, class, guild, max_length, presence, mobility, side, limit, method)] #nrow = 3,915
...
rls_summary <- rls_summary[!is.na(dLat)] #nrow = 3,676

# Save dataset for corrections and checking of species limits
write.csv(rls_summary[, !20:22][limit != "optimum"]
, "/Volumes/PHD DATA/RLS/Datasets/Derived Data/rls_summary.csv", row.names = FALSE)
...
rls_summary_bp <- ggplot(rls_summary[abs(dLat) < 5 & limit != "optimum"], 
...
# Averages
rls_means_wt <- rls_summary[abs(dLat) < 5 & limit != "optimum", 
                              .(N = NROW(dLat),
                                dLat_mean = mean(dLat),
                                dLat_sd = sd(dLat),
...
rls_sites_sf <- st_as_sf(rls_sites, coords = c("longitude", "latitude"), crs = crs(aus_pols))
...
dLat_dSST_plot <- ggplot(rls_sites[, .(dSST), by = .(site = site_code)][rls_summary, on = .(site)][!is.na(dSST) & limit != "optimum" & abs(dLat) < 5],
                         aes(x = dSST, y = dLat, col = guild)) +
  geom_point() +
  geom_smooth(method = 'glm', color = "black") +
  geom_hline(yintercept = 0, lty = "dashed") +
  facet_grid(guild + limit ~ side, scales = "free") +
  labs(title = "Site-level temperature change in relation to latitudinal displacement") +
  theme_bw()
...
# Abundance plots for temperate species
ggplot(rls_summary[guild == "temperate" & abs(pop_trend) != Inf], aes(x = class, y = pop_trend)) +
  geom_boxplot() +
  facet_grid(limit ~ side)

temp_abun_table <- rls_summary[abs(pop_trend) != Inf, 
                              .(N = NROW(pop_trend),
...
temp_abun_plot <- ggplot(temp_abun_table,
                         aes(x = guild, y = pop_trend_mean, col = guild)) +
  geom_point(position = position_dodge(width = 0.3)) +
...
pop_trend_dSST_plot <- ggplot(rls_sites[, .(dSST), by = .(site = site_code)][rls_summary, on = .(site)][!is.na(dSST)][abs(pop_trend) != Inf & guild != "optimum" & abs(dLat) < 5],
...
# Thermal niche
thermal_niche <- fread("/RLS/Datasets/Raw Data/Thermal niche.csv") #nrows = 4,465
sgi_dt <- fread("/RLS/Datasets/Raw Data/SGIs.csv") #nrows = 26,904

rls_summary_thermal <- thermal_niche[, .(taxon = `SPECIES_NAME`, upper_thermal = `95th percentile`)][rls_summary, on= .(taxon)]
rls_summary_SGI <- sgi_dt[, .(taxon = `SPECIES_NAME`, SGI)][rls_summary, on= .(taxon)]


ggplot(rls_summary_thermal[limit == "northern" & guild == "tropical" & dLat > 0],
                              aes(x = upper_thermal, y = dLat, col = guild)) +
  geom_point() +
  geom_smooth(method = 'glm', color = "black") +
  geom_hline(yintercept = 0, lty = "dashed") +
...
ggplot(rls_summary_SGI[limit == "northern" & guild == "tropical" & dLat > 0],
                              aes(x = SGI, y = dLat, col = guild)) +
  geom_point() +
  geom_smooth(method = 'glm', color = "black") +
  geom_hline(yintercept = 0, lty = "dashed") +
  facet_grid(limit ~ guild) +
...
rls_summary_plots <- ggarrange(rls_summary_bp, site_dSST_plot,
                               nrow = 1, ncol = 2,
                               widths = c(1.4, 1))
