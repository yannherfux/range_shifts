...
rls[, mean(dt)] # average time period: 7.1 years
...
for(i in site_sst_yearly[, unique(region)]){
  ...
  p1 <- ggplot(data_p1, aes(x = year, y = regional_mean)) + 
    ...
    labs(y = "Temp anomaly (°C)", x = "", subtitle = ifelse(i == "N", "a) North",
...
##### Disentangling long vs short term range shifts
```{r}
data <- rls_summary[rls[, .(dt = unique(dt)), by = .(site1 = site_code)], on = .(site1), nomatch = NULL]

data_means <- data[, .(dLat_mean = mean(dLat),
                       dLat_sd = sd(dLat),
                       dLat_se = sd(dLat) / sqrt(NROW(dLat))),
                   by = .(dt, guild, limit)]
...
ggplot() + 
  geom_point(data = data_means,
             aes(x = dt, y = dLat_mean), col = "black") +
  geom_errorbar(data = data_means, col = "black",
                aes(x = dt,
                    ymin = dLat_mean - dLat_se,
                    ymax = dLat_mean + dLat_se)) + 
  geom_line(data = data[, .(dLat = mean(dLat)), by = .(dt, region, guild, limit)], aes(x = dt, y = dLat, 
